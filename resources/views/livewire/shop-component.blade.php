                    ...
                    <div class="col-md-6">
                        @if(session('success'))
                            <div class="alert alert-success alert-dismissible"
                                 style="margin-bottom: 0%; padding-top:0.5rem; padding-bottom:0.5rem; top: -2rem;">
                                <button type="button" class="close  mt-3" data-dismiss="alert" aria-hidden="true"
                                        style="top: -16px;">×
                                </button>
                                                            ...
                                                            @foreach($manufacturers as $manufacturer)
                                                                <div class="form-check mb-2" style="font-size: 14px;">
                                                                    <input class="form-check-input" type="checkbox"
                                                                           ...
                                                                           id="manuf{{ $manufacturer->id }}"
                                                                           wire:click="checkManuf('{{ $manufacturer->id }}')">
                                                                    <label class="form-check-label"
                                                                           for="defaultCheck{{ $manufacturer->id }}">
                                                                        {{ $manufacturer->name }}
                                                                    </label>
                                                                </div>
                                                            @endforeach
                                                    ...
                                                    <a href="{{ route('product.details', ['slug' => $latestProduct->slug]) }}"
                                                       class="product-image__body">
                                                        <img class="product-image__img"
                                                             src="{{ asset('images/products').'/'.$latestProduct->image }}"
                                                             alt="{{ $latestProduct->name }} image"
                ...
                <div class="block">
                    <div class="products-view">
                        <div class="products-view__options">
                            <div class="view-options view-options--offcanvas--mobile">
                                ...
                                @if(session()->get('locale') != 'tm')
                                    <div
                                        class="view-options__legend">{{__('Showing')}} {{ $products->count() }} {{__('of')}} {{ $products->total() }} {{__('products')}}  </div>
                                @else
                                    <div class="view-options__legend">{{ $products->total() }}
                                        Harytdan {{ $products->count() }} -si görkezildi
                                    </div>
                                @endif
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-12">
                                                <img class="img-fluid" src="{{ asset('images/products').'/'.$image }}"
                                                     alt="">
                                            </div>
                                            <div class="col-12">
                                            ...
                                            <div class="col-12">
                                                <div class="form-group product__option">
                                                    <label class="product__option-label"
                                                           for="product-quantity">{{__('Quantity')}}</label>
                                                    <div class="product__actions">
                                                        <div class="product__actions-item">
                                                            <div class="input-number product__quantity">
                                                                <input id="product-quantity"
                                                                       ...
                                                                       wire:model.live="pqty">
                                                                <div class="input-number__add"
                                                                     wire:click="qtyInc"></div>
                                                                <div class="input-number__sub"
                                                                     wire:click="qtyDec"></div>
                                                            </div>
                                                        </div>
                                                        <div
                                                            class="product__actions-item product__actions-item--addtocart">
                                                            <button class="btn btn-primary"
                                                                    wire:click="store({{$id}}, '{{$name}}', {{ $sale_price }})">{{__('Add to cart')}}</button>
                                                        </div>
                                                        ...
                                                        <div
                                                            ...
                                                            @if($wproducts->contains($id))
                                                                <button type="button"
                                                                        class="btn btn-secondary btn-svg-icon"
                                                        ...
                                                        @if($cproducts->contains($id))
                                                            <div
                                                                ...
                                                                <button type="button"
                                                                        ...
                                                                        wire:click="removeCompare({{$id}}, '{{$name}}', {{ $sale_price }})">
                                                                    <svg width="16px" height="16px"
                                                                         style="fill: #ff3333;">
                                                                        <use
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        @if($sslug)
                                            <a href="{{ route('product.details', ['slug' => $sslug]) }}"
                        ...
                        <div class="products-view__list products-list" data-layout="grid-3-sidebar"
                             data-with-features="false" data-mobile-grid-columns="2" wire:ignore.self>
                            <div class="products-list__body">
                                        ...
                                        <div class="product-card ">
                                            ...
                                            <div class="product-card__image product-image">
                                                <a href="{{ route('product.details', ['slug' => $product->slug]) }}"
                                                   class="product-image__body" wire:navigate>
                                                    <img class="product-image__img"
                                            ...
                                            <div class="product-card__actions">
                                                <div class="product-card__availability">
                                                    {{__('Availability')}}: <span
                                                        class="text-success {{ $product->stock_status=='In Stock'? '': 'text-danger' }}"> <br> {{ __($product->stock_status) }}</span>
                                                </div>
                                                ...
                                                <div class="product-card__buttons">
                                                    <button class="btn btn-primary product-card__addtocart"
                                                            type="button"
                                                            wire:click="store({{$product->id}}, '{{$product->name}}', {{ $product->sale_price }})">{{__('Add to cart')}}</button>
                                                    <button
                                                    ...
                                                    @else
                                                        <button
                                                            class="btn btn-light btn-svg-icon btn-svg-icon--fake-svg product-card__compare"
                                                            type="button"
                                                            wire:click="addToCompare({{$product->id}}, '{{$product->name}}', {{ $product->sale_price }})">
                                                            <svg width="16px" height="16px">
                                                                <use
                                                                    xlink:href="{{ asset('images/sprite.svg#compare-16') }}"></use>
                                                            </svg>
                                ...
                                @empty
                                    <div class="col-sm-8 offset-2 text-center mt-5 pt-5">
                                        <p>{{ __('There are no products in this category yet.') }}</p></div>
                                @endforelse
                            </div>
                        </div>
